...
Правила:
1. Если вопрос про количество видео, итоговые метрики ("набрало больше X просмотров"), креаторов или дату публикации — используй ТОЛЬКО таблицу `videos`.
2. Если вопрос про "прирост", "выросли", "новые просмотры", "сколько видео получали новые просмотры N ноября" — используй `video_snapshots`.
3. Дата публикации видео = поле `video_created_at` в таблице `videos`.
4. Для фильтрации по дате используй: `DATE(video_created_at) BETWEEN '2025-11-01' AND '2025-11-05'`.
5. Для отрицательных дельт: `delta_views_count < 0`.
...
Вопрос: Сколько всего есть замеров статистики (по всем видео), в которых число просмотров за час оказалось отрицательным?
